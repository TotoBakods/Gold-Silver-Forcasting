...
GEN_HIDDEN_SIZE = 128
GEN_DROPOUT = 0.1
VOLATILITY_BOOST = 4.0  # Amplify returns by 4x to create a lively market
...
def generate_extension_lively(asset):
    print(f"--- Extending {asset} with {VOLATILITY_BOOST}x Volatility ---")
    dataset_config = get_dataset_config_by_asset(asset)
    # BRIDGE FIX: Use the 'extended_train' files as the source so we start exactly at 2026-04-10
    if asset == "gold":
        input_path = PROJECT_ROOT / "df_gold_dataset_gepu_extended_train.csv"
    else:
        input_path = PROJECT_ROOT / "silver_RRL_interpolate_extended_train.csv"
    ...
    # We generate a generous extension
    future_dates = pd.date_range(start=last_date + pd.Timedelta(days=1), periods=500, freq='B')
    ...
    gen_stat = gen_stat * VOLATILITY_BOOST
    ...
    # Save ONLY the Lively Future portion (matches the standard test file structure)
    output_path = PROJECT_ROOT / f"{get_output_base_name(dataset_config)}_extended_lively.csv"
    recon_df.to_csv(output_path, index=False)
    print(f"Saved LIVELY data to {output_path}")

if __name__ == "__main__":
    generate_extension_lively("gold")
    generate_extension_lively("silver")
