...
for i in range(len(free_dates)):
    with torch.no_grad():
        # MODERATE NOISE
        noise = torch.randn(1, CONFIG["gan_window"], CONFIG["gan_noise"], device=device) * 3.0
        next_stat = netG(win, noise)
        next_stat_clipped = torch.clamp(next_stat, -5.0, 5.0)
        # MODERATE JITTER
        jitter = torch.randn_like(next_stat_clipped) * 0.2
...
gold_returns = gan_stat[:, CONFIG["features"].index("Gold_Futures")]

def autocorrelation(x, lag):
    if lag == 0: return 1.0
    return np.corrcoef(x[lag:], x[:-lag])[0, 1]

print("Gold Return Autocorrelations (MODERATE NOISE):")
for l in range(1, 6):
    print(f"Lag {l}: {autocorrelation(gold_returns, l):.4f}")

print(f"Mean return: {np.mean(gold_returns):.6f}, Std: {np.std(gold_returns):.6f}")
