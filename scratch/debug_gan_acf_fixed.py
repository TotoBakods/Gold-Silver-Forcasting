...
for i in range(len(free_dates)):
    with torch.no_grad():
        # Current settings from generate_reports.py
        noise = torch.randn(1, CONFIG["gan_window"], CONFIG["gan_noise"], device=device) * 2.0
        next_stat = netG(win, noise)
        next_stat_clipped = torch.clamp(next_stat, -5.0, 5.0)
        jitter = torch.randn_like(next_stat_clipped) * 0.1
...
print("Gold Return Autocorrelations (WITH JITTER FIX):")
for l in range(1, 6):
