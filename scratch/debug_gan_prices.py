...
# Training data for scaler (up to Feb 28 to match GAN training)
...
for row in gan_stat:
    ...
    gan_prices.append(last_vals[0])

print(f"Date range: {free_dates[0].date()} to {free_dates[-1].date()}")
print(f"Start price (Mar 2 approx): {gan_prices[0]:.2f}")
print(f"Mid price (Apr 15 approx): {gan_prices[len(gan_prices)//2]:.2f}")
print(f"End price (May 29 approx): {gan_prices[-1]:.2f}")
print(f"Min: {min(gan_prices):.2f}, Max: {max(gan_prices):.2f}")
