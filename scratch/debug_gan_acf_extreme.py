...
for i in range(len(free_dates)):
    with torch.no_grad():
        # EXTREME NOISE
        noise = torch.randn(1, CONFIG["gan_window"], CONFIG["gan_noise"], device=device) * 5.0
        next_stat = netG(win, noise)
        next_stat_clipped = torch.clamp(next_stat, -5.0, 5.0)
        # EXTREME JITTER
        jitter = torch.randn_like(next_stat_clipped) * 0.5
...
print("Gold Return Autocorrelations (EXTREME NOISE):")
for l in range(1, 6):
