...
for i in range(len(free_dates)):
    with torch.no_grad():
        # Current settings for Silver (1.0/0.0)
        noise = torch.randn(1, CONFIG["gan_window"], CONFIG["gan_noise"], device=device) * 1.0
        next_stat = netG(win, noise)
        next_stat_clipped = torch.clamp(next_stat, -5.0, 5.0)
        win_update = next_stat_clipped
...
returns = gan_stat[:, CONFIG["features"].index("Silver_Futures")]

def autocorrelation(x, lag):
    if lag == 0: return 1.0
    return np.corrcoef(x[lag:], x[:-lag])[0, 1]

print("Silver Return Autocorrelations (Standard):")
for l in range(1, 6):
    print(f"Lag {l}: {autocorrelation(returns, l):.4f}")
